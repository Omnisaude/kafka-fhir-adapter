...
from kafka_fhir_adapter.resources.patient import PatientResource
...
class PatientConsumer:
    ...
    async def consume(self, messages):
        async for raw_message in messages:
            ...
            patient = PatientResource.from_dict(parsed_message)
            patient_json = json.loads(patient.to_fhir().json())

            validate_url = f"{self.fhir_server_url}/fhir/Patient/$validate"
            send_url = f"{self.fhir_server_url}/fhir/Patient/"

            validate_response = await send_validate_payload(self.app, patient_json, url=validate_url,  error_topic_name= self.topic_name_error)

            if validate_response:
                logger.info(f"Mensagem {parsed_message} validada com sucesso!")

                result = await send_payload(self.app, message=patient_json, url=send_url)
