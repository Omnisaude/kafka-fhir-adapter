...
class PractitionerRoleConsumer:
    ...
    async def consume(self, messages):
        async for raw_message in messages:
            ...
            validate_response = await send_validate_payload(self.app, practitioner_role_json, url=validate_url,  error_topic_name= self.topic_name_error)

            if validate_response:
                logger.info(f"Mensagem {parsed_message} validada com sucesso!")

                result = await send_payload(self.app, message=practitioner_role_json, url=send_url)
